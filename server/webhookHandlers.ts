...
export class WebhookHandlers {
  static async processWebhook(payload: Buffer, signature: string) {
    ...
    console.log("✅ Webhook validado | Evento:", event.type);

    if (event.type === "checkout.session.completed") {
      await this.handleCheckoutCompleted(event);
    } else {
      console.log("ℹ️ Evento recebido (não é checkout):", event.type);
    }
  }
  ...
  static async handleCheckoutCompleted(event: Stripe.Event) {
    ...
    console.log("  Amount Total:", (session.amount_total || 0) / 100, session.currency?.toUpperCase());
    console.log("  Customer Email:", session.customer_details?.email || "não disponível");
    console.log("  Customer Name:", session.customer_details?.name || "não disponível");
    ...
    try {
      ...
      if (expandedSession.line_items?.data) {
        for (const item of expandedSession.line_items.data) {
          ...
          if (product && typeof product === "object") {
            ...
            console.log("  Valor:", (item.amount_total || 0) / 100, item.currency?.toUpperCase());
            ...
            if (product.name.includes("Event Ticket")) {
              ...
              if (match) {
                ...
                console.log("  Padrão: [Data], [Hora] - [Tipo] Event Ticket");
              }
            }
          }
        }
      ...
      console.log("  Dados do cliente:", session.customer_details?.email ? "SIM" : "NÃO");
      console.log("  Line items expandidos:", expandedSession.line_items?.data ? "SIM" : "NÃO");
      console.log("=".repeat(60) + "\n");
    } catch (apiError) {
